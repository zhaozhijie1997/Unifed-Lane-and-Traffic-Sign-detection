...
class SoftmaxFocalLoss(nn.Module):
    def __init__(self, gamma, ignore_lb=255, *args, **kwargs):
        ...
        self.gamma = gamma
        self.nll = nn.NLLLoss(ignore_index=ignore_lb).cuda()

    def forward(self, logits, labels):
        scores = F.softmax(logits, dim=1).cuda()
        factor = torch.pow(1.-scores, self.gamma)
        log_score = F.log_softmax(logits, dim=1).cuda()
        log_score = factor * log_score
        
        loss = self.nll(log_score, labels).cuda()
        return loss
...
class ParsingRelationLoss(nn.Module):
    ...
    def forward(self,logits):
        ...
        return torch.nn.functional.smooth_l1_loss(loss,torch.zeros_like(loss)).cuda()
