...
import numpy as np
import cv2
from adet.data.datasets.lane_transform import find_start_pos
...
class LaneClsDataset(torch.utils.data.Dataset):
    def __init__(self, path,img_path,seg_path,  img_transform = None,target_transform = None,simu_transform = None, griding_num=200, load_name = False,
    ...
    def get_item(self):
        # l = self.list[index]
        # l_info = l.split()
        img_name, label_name = self.img_path, self.seg_path
        #if img_name[0] == '/':
            #img_name = img_name[1:]
        if label_name[0] == '/':
            label_name = label_name[1:]
        label_path = os.path.join(self.path, label_name)
        la = loader_func(label_path)
        label = la.resize((800,290),Image.NEAREST)
        label = ImageOps.expand(label,(0,180,0,0))
    ...
    def _get_index(self, label):
        ...
        # if h != 470:
            # scale_f = lambda x : int((x * 1.0/470) * h)
        ...
        sample_tmp = list(self.row_anchor)
